...
Authentication is handled via Replit Auth (OIDC) and traditional username/password for staff, secured with bcrypt hashing. Role-based access control supports Admin, Manager, CSR, Technician, and Reports roles. Admin is the highest privilege level with full control. Manager has admin-level access (can create/edit/delete staff, view logs, etc.) but cannot delete Admin accounts. The `hasAdminAccess()` helper in `routes.ts` checks for both admin and manager roles. Admins and Managers can manage staff accounts, including creation, editing, deletion, and disabling.
